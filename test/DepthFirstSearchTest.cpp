...
TEST_F(DepthFirstSearchTest, TestTopologicalSort) {
  ...
  EXPECT_NE(std::find(sorted.begin(), sorted.end(), 4),
            sorted.end()); // Ensure vertex 4 is present

  // Test topological sort on a graph with a cycle
  directed_graph.add_edge(4, 1, 1); // Create a cycle
  directed_graph.print_graph();
  EXPECT_THROW(dfs.topological_sort(),
               std::runtime_error); // Should throw an error
}
...
TEST_F(DepthFirstSearchTest, TestSingleVertex) {
  ...
  EXPECT_EQ(single_vertex_dfs.dfs(1).size(),
            1); // Should only visit the single vertex
