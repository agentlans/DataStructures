 ...
 * and perform topological sorting. It also includes functionality to check if the graph
 * is bipartite.
 * 
...
public:
     ...
     * 
     * @return A vector of vertices representing the cycle, or an empty vector if no cycle exists.
     */
...
template <class Vertex, class Weight>
void DepthFirstSearch<Vertex, Weight>::set_graph(const GraphAdjacencyList<Vertex, Weight>& g) {
    graph = &g;
}

template <class Vertex, class Weight>
std::unordered_set<Vertex> DepthFirstSearch<Vertex, Weight>::dfs(const Vertex& start) {
...
template <class Vertex, class Weight>
std::vector<Vertex> DepthFirstSearch<Vertex, Weight>::path(const Vertex& start, const Vertex& target) {
    std::unordered_set<Vertex> visited;
